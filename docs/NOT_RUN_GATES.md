...
| Static judge-exhibit deployment | PASS for published checkpoint | The public page is anonymously reachable. Current status is read from [Pages workflow history](https://github.com/zyganali-glitch/codex-control-tower/actions/workflows/pages.yml); old workflow IDs are history only. The existing old tag is a v0.1 baseline, while the v0.2.0 Pages refresh and v2 tag remain pending final verification/media. This is a static recorded exhibit, not a hosted scanner or live Codex service. |
