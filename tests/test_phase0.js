...
(async () => {
  ...
  assert.equal(FALLBACK_QUESTIONS.tr.length, 8);
  const enTemplate = readJson(path.join(ROOT, 'core', 'en', 'phase0-questions.json'));
  const trTemplate = readJson(path.join(ROOT, 'core', 'tr', 'phase0-questions.json'));
  assert.equal((enTemplate.questions || enTemplate).length, 8);
  assert.equal((trTemplate.questions || trTemplate).length, 8);
